...
- One per follow-up the milestones filed that nobody owns — or, for a bug, debt, or tidy-up when the repo has an auto-queue (`<planning-root>/<repo-slug>/_auto/`, see `yona-auto-queue`), a ticket there via `yona-auto-queue` instead of a chip.
